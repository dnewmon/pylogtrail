...
class UDPLogHandler:
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 9999, broadcast_callback: Optional[Callable[[LogEntry], None]] = None):
        """
    ...
    def _process_log_data(self, data: bytes, addr: tuple) -> None:
        ...
        try:
            ...
            if len(data) != record_length + 4:
                self.logger.warning(f"Received malformed packet from {addr}: length mismatch")
                return
                
            pickled_record = data[4:]
            
            # Unpickle the LogRecord
            log_record = pickle.loads(pickled_record)
